import { RolePermissionService } from '../../../src/services/rolePermission.service';
...
const MockedPermission = Permission as jest.MockedClass<typeof Permission>;
const MockedRolePermission = RolePermission as jest.MockedClass<typeof RolePermission>;
const MockedModule = Module as jest.MockedClass<typeof Module>;
...
describe('RolePermissionService', () => {
  ...
  describe('assignPermissionsToRole', () => {
      ...
      const mockPermissions = [
        {
          get: jest.fn((key) => {
            if (key === 'id') return 1;
            if (key === 'action') return 'create';
            if (key === 'module') return { name: 'Users' };
            return null;
          })
        },
        {
          get: jest.fn((key) => {
            if (key === 'id') return 2;
            if (key === 'action') return 'read';
            if (key === 'module') return { name: 'Groups' };
            return null;
          })
        }
      ];
    ...
    it('should throw BadRequestError if permissionIds is not an array', async () => {
      const roleId = 1;
      const permissionIds = 'not-an-array' as any;
